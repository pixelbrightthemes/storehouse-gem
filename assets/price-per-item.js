  ...
  customElements.define(
    ...
    class PricePerItem extends HTMLElement {
        ...
        this.updatePricePerItemUnsubscriber = subscribe(PUB_SUB_EVENTS.cartUpdate, (response) => {
          ...
          if (response.cartData['variant_id'] !== undefined) {
            ...
            if (isVariant && isVariant.id.toString() === this.variantId) {
              ...
              // The variant was removed from cart, quantity is 0
      ...
      updatePricePerItem(updatedCartQuantity) {
        ...
        // updatedCartQuantity is undefined when quantity is updated on product page. We need to sum entered quantity and current quantity in cart.
        // updatedCartQuantity is not undefined when quantity is updated in cart. We need to sum quantity in cart and min quantity for product.
        ...
        for (let pair of this.quantityPricePairs) {
      ...
      getVolumePricingArray() {
        ...
        this.quantityPricePairs = [];

        if (volumePricing) {
          volumePricing.querySelectorAll('li').forEach(li => {
            const quantity = parseInt(li.querySelector('span:first-child').textContent);
            const price = (li.querySelector('span:not(:first-child):last-child').dataset.text);
            this.quantityPricePairs.push([quantity, price]);
          });
        }
        this.quantityPricePairs.reverse();
      }
    }
  );
}
